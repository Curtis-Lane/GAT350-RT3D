...
uniform int celLevels = 5; // Get the number of levels to split the lighting into
uniform float celSpecularCutoff = 0.3; // Get the cutoff threshold to use for determining whether or not to show the specular highlight
uniform float celOutline = 0.3; // Get the threshold used to determine how close to orthogonal the view dir and surface normal must be to show an outline
uniform vec4 celOutlineColor = vec4(0); // Get the color used for the cel outline
...
void phong(in Light light, in vec3 position, in vec3 normal, out vec3 diffuse, out vec3 specular) {
	...
	float celIntensity = floor(intensity * celLevels) * celScaleFactor; // Calculate the cel intensity by splitting the light into levels, and then converting them back into 0.0-1.0 range
	diffuse = (light.color * celIntensity); // Change the lighting to instead use celIntensity rather than the regular intensity
...
void main()
{
	...
	for(int i = 0; i < numLights; i++) {
		...
		// Else compute the lighting as normal
